...
async function main() {
  ...
  for (const r of work) {
    ...
    try {
      ...
      if (/키가 잘못|크레딧이 소진|무료 한도|HTTP 40[13]\b|UNAUTHENTICATED|PERMISSION_DENIED|ACCOUNT_STATE_INVALID/.test(e.message)) { log(`  ❌ ${e.message}`); log('✖ 여기서 멈춥니다 (계정·키 문제, 릴스 실패로 세지 않음): ' + e.message); break; }
